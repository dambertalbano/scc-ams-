...
const UtilitysList = () => {
  ...
  }, [aToken]); // Removed getAllUtilitys from dependencies
  ...
  const handleEditClick = (admin) => {
    ...
    setCurrentAdmin(admin); // Set the selected utility's data for editing
    setIsEditing(true);
  };
  ...
  const handleSave = async () => {
    ...
    // Pass the updated utility data to the context update function
    updateUtility(currentAdmin); 
  ...
  const handleDelete = async (utilityId) => {
    try {
      await deleteUtility(utilityId); // Call the delete function from context
      getAllUtilitys(); // Refresh the utility list after deletion
    } catch (error) {
